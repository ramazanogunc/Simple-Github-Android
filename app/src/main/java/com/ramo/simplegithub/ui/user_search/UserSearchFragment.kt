...
@AndroidEntryPoint
class UserSearchFragment : BaseFragment<FragmentUserSearchBinding, UserViewModel>(
    ...
    override fun initObserver() {
        observe(viewModel.searchUsers) { users ->
            if (users.isEmpty() && viewModel.searchPage == 1)
    ...
    private fun initViews() {
        withVB {
            ...
            recyclerView.setOnItemClickListener<User> { _, _, data ->
                viewModel.goUserDetailFromSearch(data.userName)
            }
